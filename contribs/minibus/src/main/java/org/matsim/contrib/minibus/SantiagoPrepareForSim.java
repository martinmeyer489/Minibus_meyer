...
public class SantiagoPrepareForSim implements PrepareForSim{
    private static final Logger log = Logger.getLogger(SantiagoPrepareForSim.class);
     ...
     */
    @Inject
    SantiagoPrepareForSim(GlobalConfigGroup globalConfigGroup, Scenario scenario, Network network,
...
//				facilitiesFromPopulation.assignOpeningTimes(facilitiesConfigGroup.isAssigningOpeningTime(), scenario.getConfig().planCalcScore());
        ...
        System.out.println("Everything OK up to this point");
        ...
        System.out.println("Everything OK up to this point 2");
    ...
    private void createAndAddVehiclesForEveryNetworkMode() {
        ...
        for (Person person : scenario.getPopulation().getPersons().values()) {
            ...
            Map<String, Id<Vehicle>> modeToVehicle = modeVehicleTypes.entrySet().stream()
                        ...
                        if (routingMode == null) {
                            if (legs.size() == 1) {
                                ...
                                String oldMainMode = replaceOutdatedFallbackModesAndReturnOldMainMode(legs.get(0),
                                        null);
                                if (oldMainMode != null) {
                                    System.out.println("This is a second checkpoint");
                                    routingMode = oldMainMode;
                                    TripStructureUtils.setRoutingMode(legs.get(0), routingMode);
                                } else {
                                    System.out.println("This is a third checkpoint");
                                    // leg has a real mode (not an outdated fallback mode)
                                    routingMode = legs.get(0).getMode();
                                    TripStructureUtils.setRoutingMode(legs.get(0), routingMode); //here, the routingMode is set for the problematic agents...
