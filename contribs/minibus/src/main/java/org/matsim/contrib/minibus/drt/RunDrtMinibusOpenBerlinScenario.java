...
import org.matsim.contrib.minibus.drt.BerlinExperimentalConfigGroup;


import ch.sbb.matsim.config.SwissRailRaptorConfigGroup;
import org.matsim.contrib.minibus.drt.RunBerlinScenario_modified;
...
public final class RunDrtMinibusOpenBerlinScenario {

	private static final Logger log = Logger.getLogger(RunDrtMinibusOpenBerlinScenario.class);
...
//		config.controler().setLastIteration(0);
	...
	public static Controler prepareControler( Scenario scenario ) {
		...
		Controler controler = RunBerlinScenario_modified.prepareControler( scenario ) ;
...
//				bind(ScoringParametersForPerson.class).to(IncomeDependentUtilityOfMoneyPersonScoringParameters.class).in(Singleton.class);
	...
	public static Scenario prepareScenario( Config config ) {
		...
		Scenario scenario = RunBerlinScenario_modified.prepareScenario( config );
		...
		for (DrtConfigGroup drtCfg : MultiModeDrtConfigGroup.get(config).getModalElements()) {
			...
			if (drtServiceAreaShapeFile != null && !drtServiceAreaShapeFile.equals("") && !drtServiceAreaShapeFile.equals("null")) {
				...
				tagTransitStopsInServiceArea(scenario.getTransitSchedule(),
						...
						50.0); // TODO: Use constant in RunGTFS2MATSimOpenBerlin and here? Or better some kind of set available pt modes?
			}
		}
	...
	public static Config prepareConfig( AdditionalInformation additionalInformation, String [] args, ConfigGroup... customModules) {
		ConfigGroup[] customModulesToAdd = new ConfigGroup[] { new DvrpConfigGroup(), new MultiModeDrtConfigGroup(),
				new SwissRailRaptorConfigGroup(), new PConfigGroup()};
		...
		Config config = RunBerlinScenario_modified.prepareConfig( additionalInformation, args, customModulesAll ) ;
...
//		PlansConfigGroup.HandlingOfPlansWithoutRoutingMode useMainModeIdentifier =PlansConfigGroup.HandlingOfPlansWithoutRoutingMode.useMainModeIdentifier ;
